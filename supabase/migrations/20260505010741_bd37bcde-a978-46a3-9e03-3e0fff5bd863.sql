...
CREATE POLICY "Users create own quotes" ON public.quote_requests FOR INSERT WITH CHECK (
  auth.uid() = user_id AND NOT EXISTS (SELECT 1 FROM public.profiles WHERE id = auth.uid() AND is_blocked = true)
);
